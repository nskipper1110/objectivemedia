...
if [ "$FLAVOR" = "full" ]; then
    ...
    ./configure --prefix=$PREFIX \
        ...
        --enable-decoder='h263,h263p,g723_1,rawvideo' \
        --enable-encoder='h263p,g723_1,rawvideo' \
...
mkdir -p $DESTINATION_FOLDER/$FLAVOR/lib
mkdir -p $DESTINATION_FOLDER/$FLAVOR/include
cp -Rf $PREFIX/lib/* $DESTINATION_FOLDER/$FLAVOR/lib
cp -Rf $PREFIX/include/* $DESTINATION_FOLDER/$FLAVOR/include
...
if [ $TARGET == 'arm-v7n' ]; then
    ...
    OPTIMIZE_CFLAGS="-march=$CPU -Os -O3 "
    ADDITIONAL_CONFIGURE_FLAG=
    export CPPFLAGS="--sysroot=$SYSROOT -stdlib=libc++ -std=c++14"
    LIBX264_FLAGS="--disable-asm"
